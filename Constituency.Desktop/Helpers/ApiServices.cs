...
namespace Constituency.Desktop.Helpers
{
    class ApiServices
    {
        ...
        public static async Task<Response> GetListAsyncReportsVotersByCanvasParties<T>(string controller,int constituencyId, int divisionId, int canvasId,int partyId, string token)
        {
            try
            {
                HttpClientHandler handler = new HttpClientHandler()
        ...
        public static async Task<Response> GetListAsyncReportsGeneral<T>(string controller, int canvasTypeId, int canvasId, string token)
        {
            try
            {
                HttpClientHandler handler = new HttpClientHandler()
                ...
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("bearer", token);
                HttpResponseMessage response = await client.GetAsync($"api/{controller}/{canvasTypeId}/{canvasId}");
                string result = await response.Content.ReadAsStringAsync();
                if (!response.IsSuccessStatusCode)
                {
                    return new Response
